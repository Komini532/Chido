...
```bash
export CHIDO_MYSQL_CONNECTION="Server=...;Database=chido;User=...;Password=...;"
dotnet run --project Chido -- setup
```

マイグレーションの適用と初期マスタの投入を行って終了する（Discordへは接続しない）。
何度実行しても安全で、**既にあるマスタのキーには一切触れない**（無ければ入れるだけ）。

**この手順を飛ばすと Bot は起動しない。** 通常起動は起動時検証（戦闘システム 10.5）を通るが、
空のDBでは草原が存在しないため必ず失敗する。一方でテーブル作成とマスタ投入の手段が
Discordの管理者コマンドしか無いと、起動できないから初期化できず・初期化できないから
起動できない、という循環になる。`setup` はその循環を断つためにある。

稼働中にマスタの定義が増えた場合は `/admin-seed-master` を使う。投入と読み込み直しを
まとめて行う（マスタは起動時に一括で読み込んで保持しているため、行を足すだけでは
実行中のBotからは見えない）。
